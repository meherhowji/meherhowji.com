...
const pointNormalize = (p, length = 1) =>
...
const getSegmentsPathData = (segments, closed, precision) => {
  ...
  const round =
    precision < 16 ? n => Math.round(n * precisionMultiplier) / precisionMultiplier : n => n
    ...
    if (first) {
      ...
      if (inX === curX && inY === curY && outX === prevX && outY === prevY) {
        ...
        if (!skipLine) {
          ...
          const dy = curY - prevY
          parts.push(
            dx === 0 ? 'v' + round(dy) : dy === 0 ? 'h' + round(dx) : 'l' + formatPair(dx, dy),
          )
        }
      } else {
        // c = relative curveto:
...
const simplifySvgPath = (points, options = {}) => {
  ...
  return getSegmentsPathData(
    fit(
      points.map(typeof points[0].x === 'number' ? p => point(p.x, p.y) : p => point(p[0], p[1])),
      options.closed,
